...
docker run --rm -v $(pwd):/app composer create-project  laravel/laravel new-haikishare --prefer-dist "^8.0"

# プロジェクト内部に入り、appと同階層にdocker-compose.ymlを配置
mkdir new-haikishare
mv docker-compose.yml ./new-haikishare
# ymlと組み合わせて使うファイルを移動する
# webコンテナDockerfile
# apche設定ファイル
# php設定ファイル
mv docker ./new-haikishare

# コンテナ群の起動
cd new-haikishare
docker-compose up -d

docker exec -it new-haikishare_web_1 bash
chmod -R 777 /var/www/new-haikishare/storage
chmod -R 777 /var/www/new-haikishare/bootstrap
